...
top_100_significant <- t_tests %>%
  filter(p_value <= 0.05) %>%  # Keep only rows with significant p-values
  arrange(p_value) %>%         # Order by p_value, smallest to largest
  slice(1:100) %>% 
  select(Gene_name)
write.table(top_100_significant, file = "top_100_significant_proteins_names.txt",
...
proteingraph <- graph_from_data_frame(string_network)
