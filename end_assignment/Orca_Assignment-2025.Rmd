...
PDdata_PSME1 <- filter(PDdata, Gene_name == "PSME1")
colnames(PDdata_PSME1)
ggplot(data = PDdata_PSME1, aes(x = Expression, fill = Response)) +
    geom_histogram(bins = 30) +
    labs(title = "Expression of PDME1",
         xlab = "Expression",
         ylab = "count",
         fill = "Response") +
    theme_minimal()
#given you an idea of how a single protein expression behaves between two situations
```
...
#for loop or write a function?
PDdata_B2M <- filter(PDdata, Gene_name == "B2M")
ggplot(PDdata_B2M, aes(x = Expression, fill = Response)) +
...
diseased_vs_healthy %>%
  ggplot(aes(x = fold_change,
             ...
             size = significance)) +
  geom_point(shape = 21) +
  geom_hline(yintercept = -log10(0.05), # Threshhold of p values
             linetype = "dashed") +
  geom_vline(xintercept = c(-0.3, 0.3), #Threshold of DEGs
             linetype = "dashed") +
...
top_1000_significant <- t_tests %>%   
  arrange(p_value) %>%         # Order by p_value, smallest to largest (not necce)
  slice(1:1000) %>% 
  pull(Gene_name)              # Keep only rows with significant p-values
write.table(top_1000_significant, file = "top_1000_significant_proteins_names.txt",
...
#Package:igraph
#Q(Orca):what does degree mean (in graph theory)? What is a histogram of the degrees?
getwd()
string_network <- read_delim("string_interactions_short.tsv", delim="\t")

proteingraph <- graph_from_data_frame(string_network) #can add argument "directed = FALSE"
simplify(proteingraph)

#calculate degrees of the nodes
...
#show the number of nodes each gene has to other genes (most has < nodes)
```
...
#convert into an undirected graph first?
undirected <- as.undirected(proteingraph)
clusterList <- cluster_fast_greedy(undirected)
plot(clusterList, undirected,
     layout = layout_with_fr(undirected),
     vertex.size = 20,
...
cluster1graph <- read_delim("string_interactions_short_1", delim="\t")
proteingraph <- graph_from_data_frame(cluster1graph)
plot(cluster1List, undirected,
     layout = layout_with_fr(cluster1List),
     vertex.size = 20,
     vertex.label.cex = 0.2,
     vertex.label.color = "black")
```
...
library(dbplyr)
heatmap(PDdata_cluster_1, Sample, Gene_name, Expression,
        col = heat.colors(256),
        ) %>% annotation_tile(Response)
