...
PDdata_PSME1 <- PDdata %>% filter(Gene_name == "PSME1")
ggplot(PDdata_PSME1, aes(x = Expression, fill = Response)) +
  geom_histogram(binwidth = 0.5, position = "dodge", color = "black") +
  labs(title = "Expression of PSME1",
       x = "Expression Level",
       y = "Frequency") +
  scale_fill_manual(values = c("NR" = "grey", "R" = "orange")) +
  theme_minimal()
...
```{r show t_tests}
Top10_p <- head(t_tests, n = 10)
Top10_list <- PDdata %>% filter(Gene_name == Top10_p$Gene_name)
```
...
ggplot(Top10_list, aes(x = Expression, fill = Response)) +
  geom_histogram(binwidth = 0.5, alpha = 0.7, position = "dodge", color = "black") +
  labs(title = "Expression of Top10 significant genes ",
       x = "Expression Level",
       y = "Frequency") +
  scale_fill_manual(values = c("NR" = "grey", "R" = "orange")) +
...
# R based heatmap
PDdata_heatmap <- PDdata %>% filter(Gene_name %in% sig_il_genes$Gene_name)

heatmap(PDdata_heatmap, Sample, Gene_name, Expression,
        col = colorRampPalette(c("red", "white", "blue"))(100), 
        ) %>% annotation_tile(Response)
#heatmap(data, column_variable, row_variable, The values you want to visualize)
#Try using pheatmap()
```

```{r Example answer}
PDdata_1 <- PDdata %>% filter(Gene_name %in% sig_il_genes$Gene_name)

heatmap(PDdata %>% filter(Gene_name %in% sig_il_genes$Gene_name), Sample, Gene_name, Expression) %>% annotation_tile(Response)
```

```{r}
library(ComplexHeatmap)
PDdata_heatmap_matrix <- as.matrix(PDdata_heatmap)
response_colors <- c("R" = "blue", "NR" = "red")
heatmap <- Heatmap(PDdata_heatmap, name = "Expression", 
                   col = colorRampPalette(c("red", "white", "blue"))(100),
                   top_annotation = ha)
draw(heatmap)
...
```{r create a list with top 1000 proteins}

top_1000_protein_names <- diseased_vs_healthy %>%
  arrange(significance, descending = TRUE) %>%  # Assuming higher significance is better
  slice(1:1000) %>%
  pull(Gene_name)
getwd()
write.table(top_1000_protein_names, file = "top 1000 protein names.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
#if using write.csv( ), a default column name ('x') would be given
...
node_degrees <- degree(proteingraph, mode = "total")
hist(node_degrees,
     breaks = seq(0, max(node_degrees), by = 1),
     xlim = c(0, 35),
     xlab = "Degree",
     ylab = "Frequency",
     col = "lightpink",
...
clusterList <- cluster_fast_greedy(simplify(proteingraph))
...
```{r cluster1 subclustering analysis}
cluster_1 <- clusters[[1]]
head(cluster_1)
write.table(clusters[[1]], file = "proteins in cluster 1.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)

getwd()
cluster1graph <- read_delim("string_interactions_short_1.tsv", delim="\t")
proteingraph_cluster1 <- graph_from_data_frame(cluster1graph, directed = FALSE)

plot(proteingraph_cluster1,
     vertex.size = 20,
     vertex.label.cex = 0.2,
     vertex.label.color = "black")

cluster1List <- cluster_fast_greedy(simplify(proteingraph_cluster1))

```

```{r top10 significant genes in cluster 1}
cluster_1 <- as.vector(cluster_1)
cluster_1
PDdata_cluster_1 <- PDdata[PDdata$Gene_name %in% cluster_1, ]
t_tests_cluster1 <- PDdata_cluster_1 %>% 
  group_by(Gene_name) %>%
  nest() %>%
  mutate(p_value = map_dbl(data, function(x) { return (t.test(x$Expression ~ x$Response)$p.value)})) %>%
  mutate(fold_change = map_dbl(data, fold_change)) %>%
...
write.table(head(t_tests_cluster1$Gene_name, 10), file = "top_10_genes_subcluster.txt", row.names = FALSE, col.names = FALSE, quote = FALSE, sep = ",")
sigofgenes <- PDdata %>% filter(Gene_name %in% head(t_tests_cluster1$Gene_name, 10))


heatmap(sigofgenes, Sample, Gene_name, Expression,
        col = heat.colors(256),
        ) %>% annotation_tile(Response)
```

```{r subcluster analysis}
clusters_in_cluster1<- communities(cluster1List)
summary(clusters_in_cluster1)

Subcluster_1_in_cluster1 <- clusters_in_cluster1[[1]]
write.table(Subcluster_1_in_cluster1, file = "protein subcluster 1 in cluster 1.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)

Subcluster_2_in_cluster1 <- clusters_in_cluster1[[2]]
write.table(Subcluster_2_in_cluster1, file = "protein subcluster 2 in cluster 1.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)

Subcluster_3_in_cluster1 <- clusters_in_cluster1[[3]]
write.table(Subcluster_3_in_cluster1, file = "protein subcluster 3 in cluster 1.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)

Subcluster_4_in_cluster1 <- clusters_in_cluster1[[4]]
write.table(Subcluster_4_in_cluster1, file = "protein subcluster 4 in cluster 1.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)

Subcluster_5_in_cluster1 <- clusters_in_cluster1[[5]]
write.table(Subcluster_5_in_cluster1, file = "protein subcluster 5 in cluster 1.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
```
