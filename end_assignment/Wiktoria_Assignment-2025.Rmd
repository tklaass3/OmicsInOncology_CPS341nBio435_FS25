...
<<<<<<< HEAD
# Filter for PSME1 expression data
psme1_data <- PDdata %>% filter(Gene_name == "PSME1")

# Plot histogram
ggplot(psme1_data, aes(x = Expression, fill = Response)) +
  geom_histogram(alpha = 0.6, position = "identity", bins = 30) +
  labs(title = "Expression of PSME1", x = "Expression Level", y = "Count") +
  theme_minimal()
=======
# Filter for the protein 'PSME1'
PSME1_data <- filter(PDdata, Gene_name == "PSME1")

# Plot the histogram of the expression values for PSME1, with Response as fill
ggplot(PSME1_data, aes(x = Expression, fill = Response)) +
  geom_histogram(binwidth = 0.5, position = "dodge", color = "black") +
  labs(title = "Expression of PSME1 by Response",
       x = "Expression Level",
       y = "Frequency") +
  theme_minimal() +
  scale_fill_manual(values = c("NR" = "lightpink", "R" = "lightblue"))  # Custom colors for NR and R


>>>>>>> a3ff095aa47009620a41ee157937e5a6fa5ab6ea
```
...
<<<<<<< HEAD
genes_to_plot <- c("PSME1", "B2M", "TAPBP", "PTK7", "TAP2", "NXN", "STAT1", "LAP3", "PSMB5", "TAP1")  

for (gene in genes_to_plot) {
  p <- PDdata %>% 
    filter(Gene_name == gene) %>%
    ggplot(aes(x = Expression, fill = Response)) +
    geom_histogram(alpha = 0.6, position = "identity", bins = 30) +
    labs(title = paste("Expression of", gene), x = "Expression", y = "Count") +
    theme_minimal()
  
  print(p)  # Important: print() the plot object
}
=======
# Define the proteins you want to investigate
proteins_of_interest <- c("PSME1", "B2M", "TAPBP", "PTK7", "TAP2", "NXN", "STAT1", "LAP3", "PSMB5", "TAP1")

# Remove invalid Expression values
PDdata <- PDdata %>% filter(!is.na(Expression) & is.finite(Expression))

# Generating the histograms
ggplot(PDdata %>% filter(Gene_name %in% proteins_of_interest), 
       aes(x = Expression, fill = Response)) +
  geom_histogram(binwidth = 0.5, alpha = 0.7, position = "identity") +
  theme_minimal() +
  labs(title = "Histogram of Expression for Selected Proteins", x = "Expression Level", y = "Frequency") +
...
```{r}
<<<<<<< HEAD
# Write top 1000 gene names to .txt for STRING input
t_tests %>% slice(1:1000) %>% pull(Gene_name) %>% write("top1000.txt")
=======
# Extract top 1000 most significant genes by p-value
top1000_genes <- t_tests %>% slice(1:1000) %>% pull(Gene_name)

# Save to a text file, one name per line
write(top1000_genes, file = "top1000.txt")
>>>>>>> a3ff095aa47009620a41ee157937e5a6fa5ab6ea
```
...
# Histogram of degrees (connectivity)
hist(degree(proteingraph), main = "Node Degree Distribution", xlab = "Degree")

=======
proteingraph <- graph_from_data_frame(
  string_network 
)

# Simplify the graph (remove duplicates/self-loops)
proteingraph <- simplify(proteingraph)
...
"clusters <- communities(clustersList)"
=======
...
```{r extracting cluster 1}
cluster1_proteins <- clusters[[1]]
write(cluster1_proteins, file = "Wiktoria_cluster1_proteins.txt")
```
