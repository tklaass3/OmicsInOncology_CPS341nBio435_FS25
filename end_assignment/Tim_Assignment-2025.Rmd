...
ExpSetup <- read_xlsx("end_assignment/CELL-179-suppl1.xlsx", sheet="S1A", na="NaN", skip = 1)
...
PDdata_raw <- read_xlsx("end_assignment/CELL-179-suppl1.xlsx", sheet="S1D", na="NaN", skip = 1)
```
...
# Extract the top 10 most significant proteins
top10_proteins <- diseased_vs_healthy$Gene_name[order(diseased_vs_healthy$p_value)][1:10]

# Filter PDdata for the top 10 proteins
heatmap_data <- PDdata[PDdata$Gene_name %in% top10_proteins, ]

# Reshape the data to wide format, with samples as rows and genes as columns
heatmap_data_wide <- reshape(heatmap_data, 
                             idvar = "Sample", 
                             timevar = "Gene_name", 
                             direction = "wide")

# Remove the "Sample" column and convert the rest to a matrix
...
pheatmap(heatmap_matrix, 
         ...
         main = "Heatmap of Top 10 Most Significant Proteins",
         annotation_col = data.frame(Response = heatmap_data$Response[match(heatmap_data_wide$Sample, heatmap_data$Sample)]))
```
