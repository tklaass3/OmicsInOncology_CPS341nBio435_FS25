...
ggplot(PSME1_data, aes(x = Expression, fill = Response)) +
  ...
  labs(title = "Expression of PSME1 by Response",
       ...
       y = "Frequency") +
  theme_minimal() +
  scale_fill_manual(values = c("NR" = "red", "R" = "blue"))  # Custom colors for NR and R
...
sig_il_genes <- diseased_vs_healthy %>% filter(significance != "ns") %>% slice(1:50)
...
# 1.PDdata %>% filter(Gene_name %in% sig_il_genes$Gene_name)
# o	This filters the PDdata dataset to include only the top significant genes (from sig_il_genes$Gene_name).
# o	%in% is used to match gene names in PDdata with those in sig_il_genes.

# 2.	heatmap(...)
#   o This generates a heatmap where:
#        Rows represent Sample (each patient/sample).
#        Columns represent Gene_name (the 10 most significant proteins).
#        Expression values are mapped to colors in the heatmap.

# 3.annotation_tile(Response)
#   o This adds an annotation tile to indicate patient Response (R vs NR).
#   o This allows you to visually see if responders (R) and non-responders (NR) cluster differently based on protein expression.

# What this code does in summary:
# •	It filters the data for the most significant proteins.
# •	It creates a heatmap of their expression levels across samples.
# •	It adds an annotation to show which samples are R (Responders) and NR (Non-Responders).
```
