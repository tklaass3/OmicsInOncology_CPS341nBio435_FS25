...
# Loop over each protein and generate histograms
for(protein in proteins_of_interest) {
  # Filter data for the current protein
  filtered_data <- PDdata %>% filter(Gene_name == protein)
  ...
  ggplot(filtered_data, aes(x = Expression, fill = Response)) +
    ...
    theme_minimal() +
    labs(title = paste("Histogram of Expression for", protein), x = "Expression Level", y = "Frequency") +
