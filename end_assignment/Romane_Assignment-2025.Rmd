...
# filter for protein 'PSME1'
PSME1_data <- filter(PDdata, Gene_name == "PSME1")

# plot histogram with the expression values for PSME1
ggplot(PSME1_data, aes(x = Expression, fill = Response)) +
  ...
  labs(title = "Expression of PSME1 by Response",
       ...
       y = "Frequency") +
  theme_minimal() +
  scale_fill_manual(values = c("NR" = "purple", "R" = "violet"))  # Custom colors for NR and R
...
# Show top 10 proteins based on p-value
head(t_tests, 10)
```
...
# Select more proteins for visualization
proteins_to_plot <- c("PSME1", 
                      t_tests$Gene_name[2], 
                      t_tests$Gene_name[5], 
                      t_tests$Gene_name[25], 
                      t_tests$Gene_name[50], 
                      t_tests$Gene_name[100])

# Filter the data and plot
PDdata %>%
  filter(Gene_name %in% proteins_to_plot) %>%
  ggplot(aes(x = Expression, fill = Response)) +
  geom_histogram(position = "identity", alpha = 0.6, bins = 30) +
  facet_wrap(~Gene_name, scales = "free_y") +
scale_fill_manual(values = c("R" = "#4A90E2", "NR" = "#F15A60")) +
    
  labs(title = "Expression Histograms of Selected Proteins",
       x = "Expression (log2)",
       y = "Count",
       fill = "Response") +
